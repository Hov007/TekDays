						...
						<g:if test="${tekEventInstance?.organizer}">
							<li class="fieldcontain">
								...
								<span class="property-value" aria-labelledby="organizer-label">
									<g:link controller="tekUser" action="show" id="${tekEventInstance?.organizer?.id}">${tekEventInstance?.organizer?.encodeAsHTML()}</g:link>
								</span>
							...
							<li class="fieldcontain">
								...
								<g:each in="${tekEventInstance.volunteers}" var="v">
									<span class="property-value" aria-labelledby="volunteers-label">
										<g:link controller="tekUser" action="show" id="${v.id}">${v?.encodeAsHTML()}</g:link>
									</span>
								</g:each>
...
id="${s.id}">${s?.sponsor.encodeAsHTML()}</g:link></span>
						...
						<g:if test="${tekEventInstance?.task}">
							<li class="fieldcontain">
								<span id="task-label" class="property-label"><g:message code="tekEvent.task.label" default="Task"/></span>

								<g:each in="${tekEventInstance.task}" var="t">
									<span class="property-value" aria-labelledby="task-label">
										<g:link controller="task" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link>
									</span>
								</g:each>
